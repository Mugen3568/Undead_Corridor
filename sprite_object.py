...
class AnimatedSprite(SpriteObject):
    def __init__(self, game, path=None,
        ...
        if path is None:
            path = resource_path('resources/sprites/animated_sprites/green_light/0.png')
    ...
    def get_images(self, path):
        ...
        if not os.path.exists(resolved):
            ...
            images.append(surf)
            return images
        files = [f for f in os.listdir(resolved) if os.path.isfile(os.path.join(resolved, f)) and f.endswith(('.png', '.jpg', '.jpeg'))]
        if not files:
            surf = pg.Surface((100, 100))
            surf.fill('red')
            images.append(surf)
        ...
        for file_name in files:
            img = pg.image.load(os.path.join(resolved, file_name)).convert_alpha()
            images.append(img)
        return images
